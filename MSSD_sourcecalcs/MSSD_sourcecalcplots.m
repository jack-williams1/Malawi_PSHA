...
% Comparison of Zomba Fault and Zomba Graben Fault uncertainity with the MSSD
...
%index source and fault name in MSSD spreadsheet
if id >600
...
pd_sr1 = makedist('Normal','mu',MSSD_source_calcs(indx,2),'sigma',MSSD_source_calcs(indx,3)); 
pd_r2 = makedist('Lognormal','mu',MSSD_source_calcs(indx,4),'sigma',MSSD_source_calcs(indx,5));
...
%Select lower, intermediate, and upper estimates from MSSD
if id>600
...
x2_sr = [MSSD_source_calcs(indx,2)-MSSD_source_calcs(indx,3) MSSD_source_calcs(indx,2) MSSD_source_calcs(indx,2)+MSSD_source_calcs(indx,3)];
y1 = [1/3 1/3 1/3];
x2_ri = [exp(MSSD_source_calcs(indx,4)-MSSD_source_calcs(indx,5)) exp(MSSD_source_calcs(indx,4)) exp(MSSD_source_calcs(indx,4)+MSSD_source_calcs(indx,5))];
y2 = [0.16 0.68 0.16];

 
%% Load in data from SMSSD to make comparison with the MSSD
...
% Index same faults in MSSD
% Note this will only work for faults in Zomba Graben that are in both MSSD and SMSSD
for i=1:length(zomb_faults)
    if isempty(find(string(zomb_faults(i))==string(txt_raw(:,4)),1))==0
    zomb_MSSDindx(i)=find(string(zomb_faults(i))==string(txt_raw(:,4)),1);
    %for branching faults concatenate -1 so names match
    else
    zomb_MSSDindx(i)=find(string(strcat(zomb_faults(i),'-1'))==string(txt_raw(:,4)),1);  
    end
    
    %match ids with MSSD_source_calcs
    zomba_id(i)=find(num_raw(zomb_MSSDindx(i),1)==MSSD_source_calcs(:,1));
end

%find MSSD slip rates & recurrence intervals
MSSD_sr=MSSD_source_calcs(zomba_id,2); MSSD_sr_err=MSSD_source_calcs(zomba_id,3);

MSSD_ri=exp(MSSD_source_calcs(zomba_id,4));
MSSD_ril=MSSD_ri-exp(MSSD_source_calcs(zomba_id,4)-MSSD_source_calcs(zomba_id,5));
MSSD_riu=exp(MSSD_source_calcs(zomba_id,4)+MSSD_source_calcs(zomba_id,5))-MSSD_ri;
...
hold on
errorbar(categorical(zomb_faults),MSSD_sr,MSSD_sr_err,'o',...
...
legend('SMSSD','MSSD','Location','Northwest');
...
hold on
errorbar(categorical(zomb_faults),MSSD_ri,MSSD_ril,MSSD_riu,'o',...
    'LineWidth',1.2,'MarkerFaceColor','w','MarkerEdgeColor','k','Color','k');
...
% Index unique sources in MSSD spreadsheet by unique magnitudes
...
fault=vertcat(fault,num_raw(tmp2,60));%Correction for equal mag of CBF-7 and SBF-6
ia=vertcat(ia,tmp2');
...
% Index faults in MSSD_source_calcs
for i=1:length(bf_id)
    bf_indx(i)=find(bf_id(i)==MSSD_source_calcs(:,1));
end

for i=1:length(if_id)
    if_indx(i)=find(if_id(i)==MSSD_source_calcs(:,1));
end

% Index unique sections in MSSD_source_calcs
% Should equal same as num_sec1 in MSSD syncat analysis
sec_indx1=find(MSSD_source_calcs(:,1)<300); tmp_indx=zeros(length(sec_indx1),1);
for i=1:length(sec_indx1)
    tmp_indx(i)=ismember(MSSD_source_calcs(sec_indx1(i),1),num_raw(ie,2));
end

sec_indx1(find(tmp_indx==0))=[]; sec_indx2=zeros(length(sec_indx1),1);

for i=1:length(sec_indx1)
    sec_indx2(i)=find(MSSD_source_calcs(sec_indx1(i),1)==num_sec(:,1));
end

f_indx=vertcat(bf_indx,if_indx);
mf_indx=find(MSSD_source_calcs(:,1)>599);
...
%subplot(1,3,1)
clear h1 h2
h1=histogram(MSSD_source_calcs(bf_indx,2),'BinWidth',0.5,'facecolor',[0.9 0.9 0.9],'facealpha',1); hold on
h2=histogram(MSSD_source_calcs(if_indx,2),'BinWidth',0.05,'facecolor',[0.5 0.5 0.5]); hold on 
...
%Plot magnitudes 
%Note these are derived from MSSD spreadsheet
nexttile
clear h3 h4 h5
...
nexttile
h6=histogram(log10(exp(MSSD_source_calcs(sec_indx1,4))),'BinWidth',0.5,'facealpha',0.5); hold on
h7=histogram(log10(exp(MSSD_source_calcs(f_indx,4))),'BinWidth',0.5,'facealpha',0.5); hold on
h8=histogram(log10(exp(MSSD_source_calcs(mf_indx,4))),'BinWidth',0.5,'facealpha',0.5); hold on
...
%Slip rates
bfault_sr_stats = [min(MSSD_source_calcs(bf_indx,2)) mean(MSSD_source_calcs(bf_indx,2)) max(MSSD_source_calcs(bf_indx,2))];
ifault_sr_stats = [min(MSSD_source_calcs(if_indx,2)) mean(MSSD_source_calcs(if_indx,2)) max(MSSD_source_calcs(if_indx,2))];
...
sec_ri_stats = [min(exp(MSSD_source_calcs(sec_indx1,4))) mean(exp(MSSD_source_calcs(sec_indx1,4))) max(exp(MSSD_source_calcs(sec_indx1,4)))];
fault_ri_stats = [min(exp(MSSD_source_calcs(f_indx,4))) mean(exp(MSSD_source_calcs(f_indx,4))) max(exp(MSSD_source_calcs(f_indx,4)))];
mfault_ri_stats =  [min(exp(MSSD_source_calcs(mf_indx,4))) mean(exp(MSSD_source_calcs(mf_indx,4))) max(exp(MSSD_source_calcs(mf_indx,4)))];
...
legend(['Monte Carlo simulation results' newline '(Normal distribution)'],'Mean slip rate and \pm 1\sigma',...
  ['MSSD lower, intermediate,' newline ' & upper slip rate estimates'],'location','northeast');
...
legend(['Monte Carlo simulation results' newline '(Log normal distribution)'],'Mean RI and \pm 1\sigma',...
    ['MSSD lower, intermediate,' newline ' & upper RI estimates'],'location','northeast');
