%% Generate combined catalog from MSSD direct and MSSD adapt for use in PSHA codes

 %Combines MSSD and MSSD adapt into one catalog
 %Relative weightings based on cat_w as set in syncat_PSHA_MSSD_input
 %Can also make plot to compare MFD
clear all

load ('syncat_PSHA_MSSD_input','NumSimu','t_limit','cat_w','syncat_name','bg_mmin','max_bg',...
    'fault_width_weight','recurrence_type_weight');

addpath('syncat_MSSD');  addpath('syncat_adaptedMSSD'); addpath('syncat_bg');
...
%Event catalogs simplified to:
 ...
 % 6) char of G-R (MSSD adapted only)
 % 7) catalog

 
MSSD_comb = [EQCAT(:,[1,2,5,7]),zeros(length(EQCAT),1),zeros(length(EQCAT),1),ones(length(EQCAT),1)];
    load ('MSSD_Catalog_Adapted_em','StochasticEventCatalog','dm');
    
    for i=1:4
        MSSD_comb=vertcat(MSSD_comb,[StochasticEventCatalog{i}(:,[1,2,4,6,5,7]),(i+1)*ones(length(StochasticEventCatalog{i}),1)]);
    end
    
NumSimu_fs=NumSimu*5; NumSimu_bg=NumSimu;

save('MSSD_comb','MSSD_comb')
...
mo_indx1_exp=find(MSSD_comb(:,5) ==1 & MSSD_comb(:,6) ==2);
mo_indx1_char=find(MSSD_comb(:,5) ==1 & MSSD_comb(:,6) ==1);
mo_indx2_exp=find(MSSD_comb(:,5) ==2 & MSSD_comb(:,6) ==2);
mo_indx2_char=find(MSSD_comb(:,5) ==2 & MSSD_comb(:,6) ==1);
...
for ll = 1:length(all_mag_range_GR)
    ...
    allAnnualRate_comb(ll,1) = bg_AnnualRate (ll,1)+(length(find(MSSD_comb(:,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit));%combined annual rate of events
    allAnnualRate_fltw1_exp(ll,1) = bg_AnnualRate (ll,1)+(length(find(MSSD_comb(mo_indx1_exp,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(1)*recurrence_type_weight(2)));%annual rate of char events, width=1
    allAnnualRate_fltw1_char(ll,1) = bg_AnnualRate (ll,1)+(length(find(MSSD_comb(mo_indx1_char,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(1)*recurrence_type_weight(1)));%annual rate of exp events, width=1
    allAnnualRate_fltw2_exp(ll,1) = bg_AnnualRate (ll,1)+(length(find(MSSD_comb(mo_indx2_exp,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(2)*recurrence_type_weight(2)));%annual rate of events where width=1
    allAnnualRate_fltw2_char(ll,1) = bg_AnnualRate (ll,1)+(length(find(MSSD_comb(mo_indx2_char,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(2)*recurrence_type_weight(1)));%annual rate of events where width=2

    fault_syncatRate (ll,1) = bg_AnnualRate (ll,1)+(length(find(EQCAT(:,5) >= all_mag_range_GR(ll)))/(NumSimu*t_limit));%Rate from MSSD direct catalog
...
Mo_MSSD_Direct =  10.^(1.5*EQCAT(:,5)+9.05);
MoRate_MSSD_Direct = MoRate_bg_catalog +(sum(Mo_MSSD_Direct)/NumSimu);

Mo_StochasticEventCatalog_1_exp = 10.^(1.5*MSSD_comb(mo_indx1_exp,3)+9.05);
...
Mo_StochasticEventCatalog_1_char = 10.^(1.5*MSSD_comb(mo_indx1_char,3)+9.05);
...
Mo_StochasticEventCatalog_2_exp = 10.^(1.5*MSSD_comb(mo_indx2_exp,3)+9.05);
...
Mo_StochasticEventCatalog_2_char = 10.^(1.5*MSSD_comb(mo_indx2_char,3)+9.05);
...
Mo_all_MSSD =  10.^(1.5*MSSD_comb(:,3)+9.05);
MoRate_MSSD_all_MSSD = sum(Mo_all_MSSD)/NumSimu_fs;

MoRate_MSSD_comb =  MoRate_MSSD_all_MSSD  +MoRate_bg_catalog;
...
legend({['Length-limited {\it W}, Char,' newline 'M{_0} : ',num2str(MoRate_StochasticEventCatalog_1_char,2),' Nm/yr'],...
    ...
    ['MSSD Direct,' newline 'M{_0}: ',num2str(MoRate_MSSD_Direct,2),' Nm/yr'],...
    ['Combined,' newline 'M{_0}: ',num2str(MoRate_MSSD_comb,2),' Nm/yr']},...
...
%% Plot moment rate tests for MSSD Direct and MSSD adapted comparison

load syncat_comparison; load MSSD_adapted_comparison
...
plot(log10(num_MSSD(:,13)),log10(MoRate_StochasticEventCatalog{1}(:,1)),'rx',...
log10(num_MSSD(:,14)),log10(MoRate_StochasticEventCatalog{4}(:,1)),'kx','MarkerSize',12,'LineWidth',1.5);
...
%% Sample and compare Moment Rate in 50 year samples of MSSD_comb
...
for ii=1:5
 ...
 tmp_catalog=MSSD_comb(find(MSSD_comb(:,7)==ii),2:3);
...
for jj=1:length(mo_test)
    ...
    % Percent change that moment rate in MSSD_comb is lower than observed
    mo_rate_prob(jj)=length(find(mo_test{jj}(:,1)<=ins_mo_test(:,1)))/length(mo_test{jj}(:,1))*100;
    % Percent change that mmax in MSSD_comb is lower than observed
...
legend(['Direct MSSD: ',num2str(prob_text(:,1),2),'%'],['Length-limited {\it W}' newline 'Char: ' num2str(prob_text(:,2),2) '%'],...
