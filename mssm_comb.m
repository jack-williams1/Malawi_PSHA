...
addpath('syncat_MSSM');  addpath('syncat_adaptedMSSM'); addpath('syncat_bg'); addpath('misc_functions');
...
for ll = 1:length(all_mag_range_GR)
    ...
    allAnnualRate_fltw1_exp(ll,1) = (length(find(MSSM_comb(mo_indx1_exp,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(1)*recurrence_type_weight(2)));%annual rate of char events, width=1
    allAnnualRate_fltw1_char(ll,1) = (length(find(MSSM_comb(mo_indx1_char,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(1)*recurrence_type_weight(1)));%annual rate of exp events, width=1
    allAnnualRate_fltw2_exp(ll,1) = (length(find(MSSM_comb(mo_indx2_exp,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(2)*recurrence_type_weight(2)));%annual rate of events where width=1
    allAnnualRate_fltw2_char(ll,1) = (length(find(MSSM_comb(mo_indx2_char,3) >= all_mag_range_GR(ll)))/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(2)*recurrence_type_weight(1)));%annual rate of events where width=2
...
MoRate_MSSM_Direct = (sum(Mo_MSSM_Direct)/NumSimu);

Mo_StochasticEventCatalog_1_exp = 10.^(1.5*MSSM_comb(mo_indx1_exp,3)+9.05);
MoRate_StochasticEventCatalog_1_exp = (sum(Mo_StochasticEventCatalog_1_exp)/(NumSimu_fs*t_limit*cat_w(2)*fault_width_weight(1)*recurrence_type_weight(2)));

Mo_StochasticEventCatalog_1_char = 10.^(1.5*MSSM_comb(mo_indx1_char,3)+9.05);
MoRate_StochasticEventCatalog_1_char = (sum(Mo_StochasticEventCatalog_1_char)/(NumSimu_fs*cat_w(2)*t_limit*fault_width_weight(1)*recurrence_type_weight(1)));

Mo_StochasticEventCatalog_2_exp = 10.^(1.5*MSSM_comb(mo_indx2_exp,3)+9.05);
MoRate_StochasticEventCatalog_2_exp = (sum(Mo_StochasticEventCatalog_2_exp)/(NumSimu_fs*cat_w(2)*t_limit*fault_width_weight(2)*recurrence_type_weight(2)));

Mo_StochasticEventCatalog_2_char = 10.^(1.5*MSSM_comb(mo_indx2_char,3)+9.05);
MoRate_StochasticEventCatalog_2_char = (sum(Mo_StochasticEventCatalog_2_char)/(NumSimu_fs*cat_w(2)*t_limit*fault_width_weight(2)*recurrence_type_weight(1)));
...
semilogy(all_mag_range_GR,allAnnualRate_fltw1_char,'r--','LineWidth',1.5,'MarkerIndices',1:8:length(all_mag_range_GR),'MarkerSize',8); hold on; 
...
semilogy(all_mag_range_GR,bg_AnnualRate,'m-','LineWidth',1.5); hold on 
semilogy(all_mag_range_GR,allAnnualRate_comb,'b-','LineWidth',2); hold on; %MFD from event catalog

  axis([4.5 8.1 10^-5 5*10^0]);
legend({['Length-limited {\it W}, Char,' newline 'M{_0} : ',num2str(MoRate_StochasticEventCatalog_1_char,3),' Nm/yr'],...
    ['Length-limited {\it W}, G-R,' newline 'M{_0}: ',num2str(MoRate_StochasticEventCatalog_1_exp,3),' Nm/yr'],...
    ['Layer-limited {\it W}, Char,' newline 'M{_0}: ', num2str(MoRate_StochasticEventCatalog_2_char,3),'Nm/yr'],...
    ['Layer-limited {\it W}, G-R,' newline 'M{_0}: ' , num2str(MoRate_StochasticEventCatalog_2_exp,3),'Nm/yr'],...
    ['MSSM Direct,' newline 'M{_0}: ',num2str(MoRate_MSSM_Direct,3),' Nm/yr'],...
    ['Areal Sources,' newline 'M{_0}: ',num2str(MoRate_bg_catalog,3),' Nm/yr'],...
    ['Combined,' newline 'M{_0}: ',num2str(MoRate_MSSM_comb,3),' Nm/yr']},...
    'Location','southwest','FontSize',8);hold on;
set(gca,'fontsize',13);
...
bg_catalog_simu= NumSimu.*rand(length(bg_catalog),1); 
...
for ii=1:5
 ...
 tmp_catalog=vertcat(tmp_catalog,[bg_catalog_simu bg_catalog(:,4)]); 
...
%% Sample 10000 random 50 year samples of the MSSM combined catalog

%Takes ~20 mins to run, so only need to run once

cat_len=50;%length of subsampled catalog
nsim=10000;
rand_t_indx=rand(nsim,1).*(NumSimu-cat_len);%nsmim random numbers between 0-1,999,950
rand_c_indx=randi([1 5],nsim,1);

sampleAnnualRate=zeros(length(nsim),length(all_mag_range_GR));
allAnnualRate_flt=zeros(1,length(all_mag_range_GR));
allAnnualRate_bg=zeros(1,length(all_mag_range_GR));
...
for gg=1:nsim
    ...
    if gg == 200*simu_count
     ...
     tmp_catalog_indx=find(MSSM_comb(:,2)>=rand_t_indx(gg) & MSSM_comb(:,2)<(rand_t_indx(gg)++cat_len) & MSSM_comb(:,7)==rand_c_indx(gg));
     tmp_catalog_fs=MSSM_comb(tmp_catalog_indx,:);%sample catalog
     %sample bg catalog
     tmp_catalog_indx=find(bg_catalog_simu>rand_t_indx(gg) & bg_catalog_simu<(rand_t_indx(gg)+cat_len));
     tmp_catalog_bg=bg_catalog(tmp_catalog_indx,:);%sample catalog
     
     for ll=1:length(all_mag_range_GR)
        allAnnualRate_flt(ll) = length(find(tmp_catalog_fs(:,3) >= all_mag_range_GR(ll)))/cat_len;
        allAnnualRate_bg(ll) = length(find(tmp_catalog_bg(:,4) >= all_mag_range_GR(ll)))/cat_len;
     end
     
     sampleAnnualRate(gg,:)=allAnnualRate_flt+allAnnualRate_bg;
end

%save('sampleAnnualRate','sampleAnnualRate');
%% Plot analysis

load geo_mo_rate; load sampleAnnualRate
...
semilogy(all_mag_range_GR,sampleAnnualRate,'Color',[0.7 0.7 0.7],'LineWidth',0.5); hold on; %MFD from event catalog samples
p1=semilogy(all_mag_range_GR,sampleAnnualRate(1,:),'Color',[0.7 0.7 0.7],'LineWidth',0.7); hold on; %MFD from event catalog
p2=semilogy(all_mag_range_GR,allAnnualRate_comb,'b-','LineWidth',2); hold on; %MFD from event catalog
p3=semilogy(mag_check,AnnualCumNum,'r*--','LineWidth',1.5); hold on;
p4=semilogy(4:0.1:8.1,10.^(Stat_log10N(:,4)),'k-','LineWidth',1);hold on
p5=semilogy(4:0.1:8.1,10.^(Stat_log10N(:,3)),'k--','LineWidth',1);hold on
semilogy(4:0.1:8.1,10.^(Stat_log10N(:,5)),'k--','LineWidth',1);hold on
        axis square; xlabel('Magnitude'); ylabel('Annual frequency of exceedance'); 
legend([p1,p2,p3,p4,p5],{'50 year samples','Combined','SSA-GEM Catalog 1965-2015','Fitted G-R relationship','16-th/84-th percentile'},...
