...
%m_shift=400:2500;
syncat_col=vertcat([0.4 0.4 0.4],[1, 0, 0],[0 0 1],...
    [1, 0.6, 0.6],[0.6, 0.6, 1]);
...
for pp = 1:length(prob_level)
  ...
  %Plot by source for different sites and return periods
    ...
    for ii=1:num_site
        ...
        loglog(T2,mean_fractile,marker1(pp),'color',color2(ii,:),'Linewidth',1.2); hold on; grid on;
    ...
    %Plot by sites for different sources and return periods
     ...
     for ii=1:num_site
        ...
        loglog(T2,mean_fractile_fs,marker1(pp),'Color','b','Linewidth',1.2); hold on
        loglog(T2,mean_fractile_bg,marker1(pp),'Color','m','Linewidth',1.2); hold on
        loglog(T2,mean_fractile_cb,marker1(pp),'Color','k','Linewidth',1.2); hold on; grid on;
...
tiledlayout(length(prob_level),num_site)

tmpp=zeros(length(prob_level),num_site);
...
for pp = 1:length(prob_level)
   ...
   % tmp_idx=tmp_idx+1;
    ...
    %subplot(length(prob_level),num_site,tmp_idx)
       ...
       %option to determine % of ground motions that were generated by an epsilon value >3
        %start=find(F_fs >= 1-1/prob_level(pp),1,'first'); 
        %tmpp(pp,ii)=100*length(find(AMAX_GMtmp_cb(start:num_need_fs,2)>3))/(num_need_fs-start);
