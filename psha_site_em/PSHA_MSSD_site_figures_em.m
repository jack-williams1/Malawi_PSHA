...
load('syncat_PSHA_MSSD_input','prob_level','GMPEindex','num_GMPEindex','vs30_site_ref',...
    'obs_duration','lower_return_period','num_need','max_bg','par_opts',...
    't_limit','NumSimu','T','num_T','syncat_name','site','site_name');

num_syncat=5; %number of MSSD catalogs
...
%{
save('GM_20220221','AMAX_GM_bg','AMAX_GM_bg_ref','AMAX_GM_fs','AMAX_GM_fs_ref',...
...
[legh,objh] =legend([line([1,1],[1,1],'Color',syncat_col(1,:),'LineWidth',1.2),...
    ...
    {'Direct MSSD','length-limited, \it{W}, Char','length-limited, \it{W}, G-R','layer-limited \it{W}, char','layer-limited, \it{W} G-R',...
...
for pp = 1:length(prob_level)
  ...
  %Plot by source for different sites and return periods
    ...
    for ii=1:num_site
        ...
        if ii == 1; xlabel('Period (s)'); ylabel('Acceleration (g)'); axis([0.01 10 0.01 10]); axis square; title('(a) MSSD','Fontweight','normal');  end
    ...
    %Plot by sites for different sources and return periods
     ...
     subplot(2,4,8);   
     ax1=gca; set(ax1,'visible','off');
     L2=legend(h3,{["MSSD Sources: " + newline + prob_level_label(1)],["Areal Sources: " + newline + prob_level_label(1)],["Combined: " + newline + prob_level_label(1)],...
...
for pp = 1:length(prob_level)
    ...
    subplot(length(prob_level),num_site,tmp_idx)
        ...
        disagg_MSSD_JW(num_need_fs,F_fs,200,AMAX_GMtmp_cb,prob_level(pp),T(select_T),fignum_disagg,0.1,title_opt);
...
%% Mean PGA values for different MSSD event catalogs
