...
%Analyses and tests EQCAT generated from syncat_MSSM

load eqcat_test %Load direct MSSM fault record
mydir  = pwd; idcs   = strfind(mydir,'/');
addpath(mydir(1:idcs(end)-1));

load MSSM_sources
load('syncat_PSHA_MSSM_input','NumSimu','weight_SWM','t_limit')
...
legend('MSSM sources','Bg Catalog','Combined');
...
match_sec=[93,24,78,3,43,76,38,97,69,94,148,127,8,17,95,90,158,133,139,161,48,122,57,77,120,...
    22,72,116,132,4,19,84,25,114,75,137,142,118];
...
for i=1:length(num_sec1)
   ...
   if length(find(num_sec1(i,21)==num_sec1(:,21)))>=2  && isempty(find(i==match_sec))==1
        ...
        if isempty(find(i==[166,164,86,89,119,121]))==0
        tmp_indx(1)=[];
        %Correction for lengths:
        %of Mbiri with Kaporo & Sani
        elseif isempty(find(i==[140,143,61,69]))==0
        tmp_indx([1,4])=[];
        %Correction for lengths:
        %BMF  Matakataka-1&2, Lweya Central
        %Thyolo Mbewe & Metangula North 2a
        %Chirobwe Ncheu Fault Livuzeli 1&2 & Malombe Mpale
        %Thyolo North 1&2 & South Basin Fault 7a
        elseif isempty(find(i==[59,67,10,14,50,54,9,13]))==0
        tmp_indx(3)=[];
        end
...
match_fault=[25,87,58,81,98,70,47,48];
...
%% PLOT FIGURE AND SAVE CALCULATIONS FOR ANALYSIS IN 'MSSM_COMB'
