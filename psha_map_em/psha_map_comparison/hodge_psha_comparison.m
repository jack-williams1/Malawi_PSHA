...
load('syncat_PSHA_MSSM_input','Region','site_grid_interval','vs30_site_ref')
load h2015_map_values 

prob_level=[500,2475]; %change prob_level as H2015 do not consider 10% PoE in 50 years
...
clear pga_mesh
    for ii=1:length(MSSM_site)
       ...
       %Find closes PSHA values from H2015 for 500 and 2475 yr return period to those in MSSM map
         pga(ii,:) = PSA_fractile_CDF(pp,mindiff_id);
...
plabel_opt=strcat(["9% PoE in 50 years","2% PoE in 50 years"]);
...
for pp=1:length(prob_level)
    ...
    if pp==1
        ZI=ZI_500yrs; c_axis_u=0.4;
    elseif pp==2
        ZI=ZI_2475yrs;c_axis_u=0.8;
    end
    
nexttile
surf(YI(:,1)',XI(1,:)',flip(rot90(ZI,3),2)); hold on; shading interp; view(0,90); axis equal; axis(Region_2); title(label_opt(1)); subtitle({plabel_opt(pp),['vs30 = ' num2str(h2015_vs30) ' m/s']});  set(gca,'fontsize',13.5);        
...
hold on; colormap(gca,cmap); caxis([0.0 c_axis_u]); h1=colorbar; h1.Label.String = 'PGA (g)';
...
nexttile
surf(MSSM_malawi_lon_site_vec,MSSM_malawi_lat_site_vec,tmp1{pp}); hold on; shading interp; view(0,90); axis equal; axis(Region_2); title(label_opt(2)); subtitle([plabel_opt(pp),vs_val]); set(gca,'fontsize',13.5);         
plot3(MapData2(:,2),MapData2(:,1),1000*ones(length(MapData2(:,1)),1),'w-');
fill3(LakeMalawiCoord(:,2),LakeMalawiCoord(:,1),1000*ones(length(LakeMalawiCoord),1),'w');
...
nexttile
cmap1 = crameri('vik'); 
surf(MSSM_malawi_lon_site_vec,MSSM_malawi_lat_site_vec,tmp2{pp}); hold on; shading interp; view(0,90); axis equal; axis(Region_2); title([label_opt(3),'']);  set(gca,'fontsize',13.5);        
...
set(gcf,'position',[701 144 981 794]) 
...
save('h2015_map_pga','XI','YI','ZI_500yrs','ZI_2475yrs','h2015_pga_ratio','h2015_pga_ratio_ref',...
