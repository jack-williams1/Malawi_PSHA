...
addpath([mydir(1:idcs(end-1)-1),'/psha_stored_gm/20221027_GM']);
addpath([mydir(1:idcs(end-1)-1),'/psha_stored_gm/20221027_GM/AMAX_files']);
...
save('GM_MSSM_em_20221027','GM_bg','GM_bg_ref','GM_fs','GM_fs_ref','GM_cb','GM_cb_ref','-v7.3');
...
for ii = 1:length(prob_level)
    ...
    hold on; caxis([0.0 caxis_opt(ii)]); set(gca,'fontsize',9,'TitleFontSizeMultiplier',1.365);
    ...
    for ss=1:length(site_spec)
        text(site_spec(ss,2)-0.8,site_spec(ss,1)-0.2,1000,site_name(ss),'Color', 'w','FontSize',7);
    end
    ...
    plot3(MapData2(:,2),MapData2(:,1),1000*ones(length(MapData2(:,1)),1),'w-');
    plot3(LakeMalawiBorder(:,2),LakeMalawiBorder(:,1),1000*ones(length(LakeMalawiBorder),1),'k-');hold on
    hold on; caxis([0.0 caxis_opt(ii)]); set(gca,'XTick',[], 'YTick', []); set(gca,'fontsize',11);
    for jj = 1:num_traces    
    ...
    plot3(LakeMalawiBorder(:,2),LakeMalawiBorder(:,1),1000*ones(length(LakeMalawiBorder),1),'k-');hold on
    set(gca,'XTick',[], 'YTick', [])
    hold on; caxis([0.0 caxis_opt(ii)]); colormap(cmap); h5=colorbar; h5.Label.String='PGA (g)'; set(gca,'fontsize',11);
    for jj = 1:num_traces    
    %    plot3(MSSM(jj).X,MSSM(jj).Y,1000*ones(length(MSSM(jj).X),1),'r','linewidth',0.75); hold on;    
    end
   plot3(site_spec(:,2),site_spec(:,1),1000*ones(length(site_spec),1),'ks','markerfacecolor','w','markersize',7);
...
for ii = 1:3
    ...
    plot3(LakeMalawiCoord(:,2),LakeMalawiCoord(:,1),1000*ones(length(LakeMalawiCoord),1),'b-'); hold on
    %plot3(LakeMalawiBorder(:,2),LakeMalawiBorder(:,1),1000*ones(length(LakeMalawiBorder),1),'k-');hold on
    ...
    if ii==1
        ...
        for ss=1:length(site_spec)
            text(site_spec(ss,2)-tmp8(1,ss),site_spec(ss,1)+tmp8(2,ss),1000,site_name(ss),'Color', 'w','FontSize',7);
        end
    else
...
set(gcf,'position',[570 88 663 709]);
