...
for ii = 1:length(prob_level)
    ...
    hold on; caxis([0.0 0.6]); set(gca,'fontsize',9,'TitleFontSizeMultiplier',1.365);
    ...
    plot3(MapData2(:,2),MapData2(:,1),1000*ones(length(MapData2(:,1)),1),'w-');
    plot3(LakeMalawiBorder(:,2),LakeMalawiBorder(:,1),1000*ones(length(LakeMalawiBorder),1),'k-');hold on
    hold on; caxis([0.0 0.6]); set(gca,'XTick',[], 'YTick', []); set(gca,'fontsize',11);
    for jj = 1:num_traces    
    ...
    plot3(LakeMalawiBorder(:,2),LakeMalawiBorder(:,1),1000*ones(length(LakeMalawiBorder),1),'k-');hold on
    set(gca,'XTick',[], 'YTick', [])
    hold on; caxis([0.0 0.6]); colormap(cmap); h5=colorbar; h5.Label.String='PGA (g)'; set(gca,'fontsize',11);
    for jj = 1:num_traces    
        plot3(MSSM(jj).X,MSSM(jj).Y,1000*ones(length(MSSM(jj).X),1),'r','linewidth',0.75); hold on;    
    end
   plot3(site_spec(:,2),site_spec(:,1),1000*ones(length(site_spec),1),'ks','markerfacecolor','w','markersize',7);
