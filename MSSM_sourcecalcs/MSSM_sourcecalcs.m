...
% IN THE MSSM THROUGH MONTE CARLO SIMULATION THROUGH LOGIC TREE
...
load('syncat_PSHA_MSSM_input','dip_w','hwf_w','ext_w')
...
%link rows in num_raw with MSSM_sources variables and basin

MSSM_id = vertcat(num_fault(:,1),num_sec1(:,1),num_multi_fault(:,1));
basin_indx=zeros(length(MSSM_id),1);

for i=1:length(MSSM_id)
    
    if MSSM_id(i,1) >600
        MSSM_id(i,2)=find(MSSM_id(i,1)==num_raw(:,83),1);
    elseif MSSM_id(i,1) >300
        MSSM_id(i,2)=find(MSSM_id(i,1)==num_raw(:,81),1);
    else
        MSSM_id(i,2)=find(MSSM_id(i,1)==num_raw(:,82),1);
    end
    
    %basin index for hanging wall flexure correction
    if isempty(find(string(txt_raw(MSSM_id(i,2)+2,3))==string(txt_hwf(:,1))))==0    
        basin_indx(i)=find(string(txt_raw(MSSM_id(i,2)+2,3))==string(txt_hwf(:,1)));
    end
end
...
% Weightings are set in PSHA_MSSM_input
...
mean_sr =zeros(length(MSSM_id),1); std_sr=zeros(length(MSSM_id),1);
mean_r=zeros(length(MSSM_id),1); std_r=zeros(length(MSSM_id),1);
range_sr =zeros(length(MSSM_id),2); range_r =zeros(length(MSSM_id),2);
constraint_test=zeros(length(MSSM_id),1);

simu_count = 1; nbindx=0;

ss=1;

for ss=1:length(MSSM_id)

    if ss== 40*simu_count
        disp(['MSSM source: ',num2str(ss),' out of ',num2str(length(MSSM_id))]);
...
sed =zeros(n_sim,1); ri =zeros(n_sim,1);
  ...
  id=MSSM_id(ss,1); ii=MSSM_id(ss,2); 
...
MSSM_source_calcs=[MSSM_id(:,1),mean_sr,std_sr,log(mean_r),std_r];
save('MSSM_source_calcs.mat','MSSM_source_calcs','range_r','range_sr','slip_rate_nb','constraint_test');

% Write data into MSSM spreadsheat

[num_sec,txt_sec,raw_sec]=xlsread('MSSM.xlsx',2);
[num_fault,txt_fault,raw_fault]=xlsread('MSSM.xlsx',3);
[num_mfault,txt_mfault,raw_mfault]=xlsread('MSSM.xlsx',4);
...
indx=zeros(length(MSSM_source_calcs),1);
simu_count=1;

for i=1:length(MSSM_source_calcs)
    
    if i== 20*simu_count
        disp(['MSSM source: ',num2str(i),' out of ',num2str(length(MSSM_source_calcs))]);
        simu_count = simu_count + 1;
    end
    
    %Write data for fault sections
    if MSSM_source_calcs(i,1)<300
       indx(i)=find(MSSM_source_calcs(i,1)==num_sec(:,1));  
       writematrix(MSSM_source_calcs(i,2:3),'MSSM.xlsx','Sheet','SectionGeometry','Range',['AA',num2str(indx(i)+1)]);
       writematrix([exp(MSSM_source_calcs(i,4)-MSSM_source_calcs(i,5)),exp(MSSM_source_calcs(i,4)), exp(MSSM_source_calcs(i,4)+MSSM_source_calcs(i,5))],...
       'MSSM.xlsx','Sheet','SectionGeometry','Range',['W',num2str(indx(i)+1)]);
    
    elseif MSSM_source_calcs(i,1)>600
       indx(i)=find(MSSM_source_calcs(i,1)==num_mfault(:,1));
       writematrix(MSSM_source_calcs(i,2:3),'MSSM.xlsx','Sheet','MultiFaultGeometry','Range',['F',num2str(indx(i)+1)]);
       writematrix([exp(MSSM_source_calcs(i,4)-MSSM_source_calcs(i,5)),exp(MSSM_source_calcs(i,4)), exp(MSSM_source_calcs(i,4)+MSSM_source_calcs(i,5))],...
       'MSSM.xlsx','Sheet','MultiFaultGeometry','Range',['L',num2str(indx(i)+1)]);
  
    else
      indx(i)=find(MSSM_source_calcs(i,1)==num_fault(:,1)); 
      writematrix(MSSM_source_calcs(i,2:3),'MSSM.xlsx','Sheet','FaultGeometry','Range',['O',num2str(indx(i)+1)]);
      writematrix([exp(MSSM_source_calcs(i,4)-MSSM_source_calcs(i,5)),exp(MSSM_source_calcs(i,4)), exp(MSSM_source_calcs(i,4)+MSSM_source_calcs(i,5))],...
      'MSSM.xlsx','Sheet','FaultGeometry','Range',['T',num2str(indx(i)+1)]);
    end    
end
