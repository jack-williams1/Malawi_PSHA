...
load ('syncat_PSHA_MSSM_input','weight_SWM');
...
set(gca,'fontsize',13); axis([5 num_MSSM(indx,12)+0.2 10^-6 10^-2]); hold on;
xlabel('Magnitude'); ylabel('Annual frequency'); grid on; axis square;
...
if ff>600 %For multi_fault, plot results from MSSM and eqcat

    m1=semilogy(num_multi_fault(m_flt_id,10),(1/num_multi_fault(m_flt_id,13)),'k*'); hold on; %Plot M-F from MSSM for multi_fault
    
    for i=1:length(flt_id)
        f1(i)=semilogy(num_fault(flt_indx(i),18),(1/num_fault(flt_indx(i),21)),'LineWidth',1.3,'*','Markersize',8,'MarkerEdgeColor',[0 0.5 0]); hold on; %Plot M-F from MSSM for fault
    ...
    for i=1:length(sec_id)
      ...
      if i>1  %avoid duplication for legend indexing
        ...
        legend([s1 f1 m1 a1(1) a1(2) a2(1) a2(2)],{['MSSM MultiFault', 'MSSM Fault','MSSM Section','Adapted char events:' newline 'length-limited width'],['Adapted G-R events:' newline 'length-limited width'],...
...
%% Plot all theortical model curves for the MSSM source
figure(7)
