...
% All fault source info from num_MSSM, where information for multi-faults
...
load MSSM_sources; load SZ_final;
load('syncat_PSHA_MSSM_input','bg_mmin','NumSimu','t_limit');

%need to scale catalog generation for Poisson process for high activity G-R faults
adapted_scaling_factor=4; 
t_limit_a=t_limit/adapted_scaling_factor;
NumSimu_a=NumSimu*adapted_scaling_factor;

%% Select MSSM Fault info and magnitude recurrence characteristics

Mx = num_MSSM(:,12);%Max mag from Leonard scaling relationships, but this is varied
...
%Extract slip rate from num_MSSM in m/yr 
Slip_rate_option = num_MSSM(:,6)*10^-3;
...
for ff= 1:length(num_MSSM) %loop through each fault/multi_fault
    ...
    for ii = 1:2
        ...
        if ii == 1
            Af = num_MSSM(ff,9)*10^6;%Width controlled by source length
        elseif ii == 2
            Af = num_MSSM(ff,11)*10^6;%Width controlled by 35 km thick seismogenic layer
        end
    ...
    alpha_var = horzcat(alpha_var,ones(height(alpha_var),1)*num_MSSM(ff,1));
...
disp(['Total duration of simulation: ',num2str(NumSimu_a*t_limit_a),' years']);
...
for ww=1:2 %select of mfd
    for mm =1:2 
        ...
        StochasticEventCatalog{count_case}=zeros(4*NumSimu_a,8);
    end
end
...
for ss=1:length(syncat_opt)
    ...
    global_count = 0; Num_event_adapted_tmp=zeros(NumSimu_a,1);
    tmp_count = 1;
...
for ii = 1:NumSimu_a%Run event catalog for duration set by N 
    
    if ii == tmp_count*200000
        tmp_count = tmp_count + 1;
        clock_new = cputime;
        disp(['Current simulation cycle : ',num2str(ii*t_limit_a),' out of ',num2str(NumSimu_a*t_limit_a),' & Required time is: ',...
    ...
    if min(occurrence_time) <= t_limit_a
            
       f_indx = find(occurrence_time <=t_limit_a);%index faults for which event occurred
    ...
    % 8    ) Reccurrece parameter
         ...
         StochasticEventCatalog{ss}(global_count-length(f_indx)+rr,1:8) = [global_count-length(f_indx)+rr ii*t_limit_a occurrence_time(f_indx(rr)) m{rr} fault_width num_MSSM(f_indx(rr),1) recurrence_type recurrence_para];
       end %end rr loop
            
   Num_event_adapted_tmp(ii,1) = length(f_indx); %Num_event for each simulation in catalog 
...
save('MSSM_Catalog_Adapted_em','StochasticEventCatalog','Num_event_adapted','YC85SeismicMoment_char','YC85SeismicMoment_exp',...
