...
for ii = 1:NumSimu %Run event catalog for duration set by N 
    ...
    % 8    ) Reccurrece parameter
         ...
         StochasticEventCatalog{ss}(global_count-length(f_indx)+rr,1:8) = [global_count-length(f_indx)+rr ii occurrence_time(f_indx(rr)) m{rr} fault_width num_MSSM(f_indx(rr),1) recurrence_type recurrence_para];
       end %end rr loop
...
save('MSSM_Catalog_Adapted_em','StochasticEventCatalog','NumSimu','Num_event_adapted','t_limit','YC85SeismicMoment_char','YC85SeismicMoment_exp',...
