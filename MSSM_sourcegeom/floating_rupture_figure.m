...
% Used as Fig 4 in PSHA paper

clear all
%load MSSD_Catalog_Adapted running for mutliple faults

mydir  = pwd; idcs   = strfind(mydir,'/');
addpath(mydir(1:idcs(end)-1));
addpath([mydir(1:idcs(end)-1) '/syncat_adaptedMSSD']);
...
flt_indx = find(num_MSSD(:,1)==source_id); 
...
f_id = unique(source_pts1(:,4));  mssd_indx =  find(num_MSSD(:,1)==source_id);%for indexing MSSD
...
axis equal; daspect([0.01 0.01 1.75]); zlim([0 35]); 
xlabel('Longitude'); ylabel('Latitude'); zlabel('Depth (km)');hold on,
ax=gca; ax.ZDir = 'reverse'; ax.XDir = 'reverse'; ax.YDir = 'reverse';  set(get(gca,'xlabel'),'rotation',+80); 
set(gca,'fontsize',13); view((num_MSSD(mssd_indx,4)-100),(90-dip)) %View parallel to fault strike
...
string_2 = {['Length-limited {\it W} total events: ' num2str(event_num1) newline 'Crust-limited {\it W} total events: ' num2str(event_num2)]}; hold on
set(cbar.Title,{'String','Rotation','Position'},{string_2,0,[0 300]}); hold on;
for g=1:length(geom_indx)
plot3(fault_geometry_points_2{geom_indx(g)}(:,2),fault_geometry_points_2{geom_indx(g)}(:,1), fault_geometry_points_2{geom_indx(g)}(:,3),'k-'); hold on
end
axis equal; daspect([0.01 0.01 1.7]); xlabel('Longitude'); ylabel('Latitude'); zlabel('Depth (km)');hold on,
ax=gca; ax.ZDir = 'reverse'; ax.XDir = 'reverse'; ax.YDir = 'reverse';  set(get(gca,'xlabel'),'rotation',+80);
set(gca,'fontsize',13); view((num_MSSD(mssd_indx,4)-100),(90-dip))
