...
flt_num = [308 304;... % Elephant-Marsh (308) & Panga-2 (304)
            ...
            369 386;... % Usisya North (370) & Lipichili  (386)
            370 386;... % Usisya Main (369) & Lipichili  (386)
...
% Finds area of flt2
   ...
   [area2(ii),flt2a{ii},newpoints]=faultintersect(flt_indx(ii,1),flt_indx(ii,2),geometry_top,geometry_bottom,fault_geometry_points,faultgrid_wgs84,fault_geom_,num_fault,fig_option);
   
   %Replace fault geom
    faultgrid_wgs84{flt_indx(ii,2)}=[];
    fault_geometry_points{flt_indx(ii,2)}=[];
    faultgrid_wgs84{flt_indx(ii,2)}=flt2a{ii};
    fault_geometry_points{flt_indx(ii,2)}=newpoints;
end
...
%% Save MSSM geometry to csv file
tmp=cell2mat(fault_geometry_points_1');

%Remove Duplicate fault geometries:
%South Basin Fault 11, South Basin 5b&5c, South Basin Fault 12b, South Karonga 2,Sabi 2
%Katseula 2,Karonga 2,North Basin 9b, North Basin 4b
dup_faults=[350,353,354,362,395,396,401,405,413,416]; clr_indx=0;

for ii=1:length(dup_faults)
    
    tmp_indx=find(dup_faults(ii)==tmp(:,4));
    clr_indx=vertcat(clr_indx,tmp_indx);
end

clr_indx(1)=[]; tmp(clr_indx,:)=[];

xref=readtable('id_xref.xlsx');%cross ref between id numbers with and without duplicate faults
for ii=1:height(xref)
    tmp_indx=find(xref.unofficial_id(ii)==tmp(:,4));
    tmp(tmp_indx,5)=xref.MSSM_id(ii);
end

writetable(table(tmp(:,1),tmp(:,2),tmp(:,3),tmp(:,5),'VariableNames',{'lat','lon','depth','MSSM_id'}),'MSSM_source_geometry.csv');