...
for i=1:length(flt_2)
   ...
   [area2,flt2a{i},tmp,newpoints]=faultintersect(flt_indx1,flt_indx2(i),geometry_top,geometry_bottom,fault_geometry_points,faultgrid_wgs84,fault_geom_,num_fault,fig_option);
    %need to add a tmp, as newpoints_f output in fault intersect function
    %doesn't work with patch plotting
   ...
   hold on
        ...
        for ii=1:length(newpoints)   
            patch('XData', newpoints{ii}(:,2),'YData', newpoints{ii}(:,1),'ZData', newpoints{ii}(:,3),'FaceColor',col{i});hold on 
        end; hold on;
