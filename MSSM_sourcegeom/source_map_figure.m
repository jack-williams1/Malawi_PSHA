...
%Central Basin Faults
cbfault_indx=find(faultgrid_wgs84_num(:,4)>366 & faultgrid_wgs84_num(:,4)<392);

for i=1:length(cbfault_indx)
...
plot3(faultgrid_wgs84_num(cbfault_indx(i),2),faultgrid_wgs84_num(cbfault_indx(i),1),-1*faultgrid_wgs84_num(cbfault_indx(i),3),'square',...
    'LineWidth',0.4,'Color',grid_color(cbfault_indx(i),2:4)) 
hold on
end
...
tmp_indx1=find(id_xref.unofficial_id>366 & id_xref.unofficial_id<392);
tmp_indx2=id_xref.MSSM_id(tmp_indx1);
tmp_indx3=id_xref.MAFD_id(tmp_indx1);

%Plot MSSM polygons
for kk=1:length(tmp_indx2)
    tmp_indx4=find(source_geom(:,4)==tmp_indx2(kk));
    plot3(source_geom(tmp_indx4,2),source_geom(tmp_indx4,1),-1.*source_geom(tmp_indx4,3),'k-','linewidth',1.5);hold on
...
for kk = 1:num_traces 
    [LAT,LON] = minvtran(utms,MAFD(kk).X,MAFD(kk).Y);
    if contains(MAFD(kk).fault_name,removed_faults)==1  && MAFD(kk).fault_name~="South Basin Fault 4"
        %map faults not included in the MSSM as black
        plot3(LON,LAT,ones(length(LON),1),'k-','linewidth',1.5); hold on;
    elseif isempty(find(MAFD(kk).MAFD_ID==tmp_indx3))==0
        plot3(LON,LAT,ones(length(LON),1),'r-','linewidth',1.5); hold on; 
    end
end

ylabel('Latitude','FontSize',16); xlabel('Longitude','FontSize',16); zlabel('Depth (km)');
set(get(gca,'ylabel'),'rotation',290); set(get(gca,'xlabel'),'rotation',15);zticks([-35 0])
zticklabels({'-35','0'});
%Plot DEM, border & lake

%hillshade generated from GDEM in gis_file folder with stored xy data
malawi_hillshade = imread('cb_hillshade.jpg'); 
load hillshade_xy_data_CB %load coordinate locations
floor_image = hgtransform('Matrix',makehgtform('translate',[0 0 -0]));
im=image('XData',Lon_Malawi,'YData',Lat_Malawi,'CData',malawi_hillshade,'Parent',floor_image); colormap(gray);
%Add box around image and make tranparent
im.AlphaData=0.6; plot3([33.8 33.8 35 35 33.8],[-12.1 -10.6 -10.6 -12.1 -12.1],[0 0 0 0 0],'k-','linewidth',1);
%plot3(MapData2(:,2),MapData2(:,1),zeros(length(MapData2(:,1)),1),'w-') ; hold on  
plot3(LakeMalawiCoord(:,2),LakeMalawiCoord(:,1),zeros(length(LakeMalawiCoord(:,1)),1),'k') ; hold on  

xlim([33.8 35]); ylim([-12.1 -10.6]); box on; grid on;
ax=gca; daspect([0.01 0.01 1]); zlim([-35 5]); hold on

%view(0,90) %birdseye view
view(161,45) %oblique view
set(gca,'fontsize',13)
set(gcf,'position',[681 151 980 828])
